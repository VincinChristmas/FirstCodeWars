...
let finalCapture = []
  for (let i = 0; i < arr.length; i++) {
    captureLength.push(arr[i].length)
    captureLength = captureLength.sort(compareNumbers)
  }
  for (let i = 0; i < arr.length; i++) {
    finalCapture.push(arr[i].substr(0, captureLength[0]))
  }
  return finalCapture
}
...
function colorOf(r,g,b){
  ...
  if (r < 16) {
    first = `${0}${r.toString(16)}`
  }
  if (g < 16 ) {
    second = `${0}${g.toString(16)}`
  }
  if (b < 16) {
    third = `${0}${b.toString(16)}`
  }
...
Create an array (which you will eventually return). Then, traverse obj, checking each key and value.
 If the length of the key is equal to 5, then push the key to your array. Separately, if the length of the 
...
function giveMeFive(obj){
  //coding here
  let howLong = []
  for (var thisOne in obj) {
    if (thisOne.length === 5) {
      howLong.push(thisOne)
    }
    if (obj[thisOne].length === 5) {
      howLong.push(obj[thisOne])
    }
...
Behaviour
You need to write a loop statement within the function that loops n times. Each time through the 
loop it will add one * to str, alternating on which side it is padded: the first time will add a * to the 
...
days that are in month. There is no need for input validation: month will always be greater than 0 and 
...
parameter:n, n is the number of hotdogs a customer will buy, different numbers have different prices
...
statement if...else, if val value is false (val==false or it can convert to false), should return a 
string "false", if not, return a string "true".
...
function trueOrFalse(val){
  if (val ==false || val == undefined || val == 0 || val == -0 || val == "" || val == NaN || val == null) { 
    return "false"
    }
else {
return "true"
...
making these function return value equal to 100. 
...
/*
The function pickIt accepts 1 parameter, arr, which is an array of numbers. We need to traverse arr 
by using a for loop. If an element is an odd number, push it to the odd array, and if it's an even number, push it to the evenarray.
...
baby of 5 months, will have an ascribed ‘age’ of 0. Return a
 new array (a tuple in Python) with [youngest age, oldest age, difference between the youngest and oldest age].
*/
...
function differenceInAges(ages){
  ...
  let sortedOut = ages.sort(compareNumbers) 
  let youngest = sortedOut[0]
  let oldest = sortedOut[sortedOut.length-1]
  let theDifference = oldest - youngest
  return [youngest, oldest, theDifference]
}
...
function addLength(str) {
  ...
  let fullBox = []
    newStr.forEach((finalStr) => {
       fullBox.push(`${finalStr} ${finalStr.length}`)
    })
  return fullBox
  }
...
returns true if the sum of the squares of each element in a is strictly greater than the sum of 
...
function distinct(a) {
  let nonDuplicates = []
  for (let i = 0; i < a.length; i++) {
    if (nonDuplicates.includes(a[i]) === false) {
...
function gooseFilter (birds) {
  ...
  let nogeeseAllowed = [];
  birds.forEach((bird) => {
    if (geese.includes(bird) === false) {
      nogeeseAllowed.push(bird)
    }
  })
